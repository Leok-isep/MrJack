...
public class Board {
    ...
    public ArrayList<ArrayList<District>> board = new ArrayList<>(3);
    ...
    public void init() {
        ...
        tokens = new ArrayList<>(Arrays.asList(Token.values()));
        ...
        board.get(0).get(0).setDirection(Direction.West);
        //Watson doit être face à un mur
        board.get(0).get(2).setDirection(Direction.East);
        //Toby doit être face à un mur
        board.get(2).get(1).setDirection(Direction.South);
    ...
    public void rotate(){
        ...
        District district;
        district = board.get(quartier1Line).get(quartier1Column);
        district.rotation();
    }
    ...
    public void exchange(){
        ...
        District permut;
        permut = board.get(quartier1Line).get(quartier1Column);
        board.get(quartier1Line).set(quartier1Column, board.get(quartier2Line).get(quartier2Column));
        board.get(quartier2Line).set(quartier2Column, permut);
    ...
    public void tokensTurnOdd(){
        tokens.get(0).setIsHead((Math.random() < 0.5));
        tokens.get(1).setIsHead((Math.random() < 0.5));
        tokens.get(2).setIsHead((Math.random() < 0.5));
        tokens.get(3).setIsHead((Math.random() < 0.5));
    }

    public void tokensTurnPair(){
        for (int i = 0;i<tokens.size();i++){
            if(tokens.get(i).isHead()){
                tokens.get(i).setIsHead(false);
            }else{
                tokens.get(i).setIsHead(true);
            }
        }
    }
    ...
    public void endGame(){
        ...
        if (isMrJackInFrontOf == 0) {
            ...
            for(int i = 0; i<numberDistrictsView; i++){
                ...
                board.get(j).get(k).setIsSuspect(false);
            }
    ...
    public String gameBoard() {
        ...
        return "Voici en temps réel le plateau de jeu :" + gameBoard
                ...
                + "\nVoici les quatre jetons pour le tour " + gameTurn +" : " + tokens
